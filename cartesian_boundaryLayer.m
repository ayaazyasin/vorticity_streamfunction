...
% Set up for boundary layer boundary conditions
% Ayaaz Yasin - Nov 22, 2024
clear; clc; close all;
...
alpha   = 2;            %[1/s],         du/dy at top boundary
...
while true
    ...
    %outlet zero-gradient boundary at right
    sNew(end,:) =  sNew(end-1,:);   
    u(end,:) = u(end-1,:);
    v(end,:) = v(end-1,:);

    %stress continuity boundary at top
    u(:,end) = dy*alpha + u(:,end-1);

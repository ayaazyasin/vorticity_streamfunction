...
% Set up for corner flow boundary conditions
% Ayaaz Yasin - Nov 23, 2024
clear; clc; close all;
...
relax   = 0.5;          %[-],           vorticity relaxation factor
...
v_top   = uMax/xMax;        v_left  = 0;        v_right     = 0;    v_bottom    = 0;
s_top   = xMax-x;   s_left  = uMax*y*xMax;    s_right     = 0;    s_bottom   = min(y);
...
while true
    ...
    wNew(end,:) =  (v(end,:) - v(end-1,:))/dx;      %right
...
contourf(X,Y,s', 70, 'EdgeColor','none'); colormap(jet), colorbar(gca, "southoutside");
axis equal; xlim([0,x(end)]);ylim([0,y(end)])
hold off; set(gca, 'FontSize',14, 'FontWeight', 'bold')
